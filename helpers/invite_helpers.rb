class InviteHelper
  ...
  def self.available_invites_from_user(id)
    invites = Invite.where('reciver_id = ? and status = ?', id.to_s, '0')
  ...
  def self.invitation_successed?(event_id, emails, user_id)
    ...
    return true unless results.include?(false)
  ...
  def self.create_invites_and_return_success(emails, event_id, user_id)
    emails.map do |email|
      ...
      invite = Invite.new({ event_id: event_id.to_i, sender_id: user_id, reciver_id: reciver.id })
